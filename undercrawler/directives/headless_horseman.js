;(function(window) {
    ...
    var _clickCheckedProperty = _HH_PROPERTY + 'clickChecked';
    var _mouseoverCheckedProperty = _HH_PROPERTY + 'mouseoverChecked';
    ...
    // Cleans up any mess that Headless Horseman created.
    function cleanup() {
        var p = new Promise();

        debugLog("cleanup: clearing custom element attributes.");

        var selector = [
            _clickCheckedProperty,
            _mouseoverCheckedProperty,
        ].map(function (item) {return '[' + item + ']'}).join();

        var elementsToClear = document.querySelectorAll(selector);

        for (var i = 0; i < elementsToClear.length; i++) {
            var elementToClear = elementsToClear[i];
            elementToClear.removeAttribute(_clickCheckedProperty);
            elementToClear.removeAttribute(_mouseoverCheckedProperty);
        }
    ...
    function clickXhrElement() {
        ...
        for (var index in elements) {
            ...
            if (!element.hasAttribute(_clickCheckedProperty) && clickLikelyTriggersXhr(element)) {
                found = true;
            }

            element.setAttribute(_clickCheckedProperty, 'ok');
    ...
    function findElementsWithListener(eventName) {
        ...
        debugLog('findElementsWithListener: ' + registeredListeners.length +
        ...
        debugLog('findElementsWithListener: ' + attributeListeners.length +
    ...
    function mouseoverXhrElement() {
        ...
        for (var index in elements) {
            ...
            if (!element.hasAttribute(_mouseoverCheckedProperty) && mouseoverLikelyTriggersXhr(element)) {
                found = true;
            }

            element.setAttribute(_mouseoverCheckedProperty, 'ok');
        ...
        debugLog('mouseoverXhrElement: Mousing over', element);
    ...
    function patchAddEventListener() {
        debugLog('patchAddEventListener: patching listener.');
    ...
    function removeOverlays() {
        var p = new Promise();
        debugLog("removeOverlays: Running overlay remover.");
        ...
        divs.forEach(function (div) {
            ...
            if (divZIndex != 'auto' &&
                ...
                debugLog('removeOverlays: Div looks like an overlay:' +
                          ' div z-index(' + divZIndex +
                          ') > windox z-index(' + baseZIndex +
                          ') and div area(' + divArea +
